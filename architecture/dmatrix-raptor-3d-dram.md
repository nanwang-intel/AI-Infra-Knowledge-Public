...
***引言——d-Matrix 披露的 Raptor 3D DRAM，把 32 GB 定制 DRAM 与 N4 计算逻辑做 face-to-face 堆叠，宣称提供 100 TB/s 带宽和 0.37 pJ/bit 的垂直 I/O 能效。表面问题是它是否比 HBM 更快；真正的问题是，删除片外内存边界之后，瓶颈会迁移到哪里。***
...
**1. 架构重构：Raptor 删除的不是 DRAM，而是 memory-to-compute 的片外路径**

2. 数字反推：100 TB/s、840 个 bank 与 0.37 pJ/bit 能否互相闭合

3. 物理代价：logic-on-top、刷新、ECC、良率与容量密度如何交换

4. Workload 审计：为什么 MoE decode 可能受益，3T 与 1M context 仍缺什么

5. 系统 Deep Dive：多单元扩展，以及权重平面与 KV 状态平面如何结合

6. 验证路径：哪些器件、封装和系统数据能够证实或推翻核心判断
...
## II. 数字反推：100 TB/s 与 0.37 pJ/bit
...
## V. 系统 Deep Dive：从 32 GB 单元到模型级服务
...
其中 $r_{reserve}$ 包含 spare、坏块隔离、热备、碎片和滚动升级预留。因此，47 只是“所有 bit 都能放权重”的数学下限，不是可运维系统的卡数。
...
$\alpha\ge1$ 表示路由元数据、对齐、重传和拓扑复制开销。举一个只用于量级判断的假设：$L_{MoE}=64$、$k=2$、$D=16{,}384$、$b=2$、$\alpha=1$，得到约 8 MiB/token；在 1,000 token/s aggregate rate 下约为 8.4 GB/s。
